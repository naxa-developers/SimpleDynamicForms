...
public final class ToastUtils {
    ...
    private static int yOffset = (int) (64 * SimpleDynamicForm.getInstance().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes int layoutId) {
        LayoutInflater inflate = (LayoutInflater)SimpleDynamicForm.getInstance().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    private static void show(@StringRes int resId, int duration) {
        show(SimpleDynamicForm.getInstance().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(SimpleDynamicForm.getInstance().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        ...
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
            if (view != null) {
                sToast = new Toast(SimpleDynamicForm.getInstance());
                sToast.setView(view);
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(SimpleDynamicForm.getInstance(), spannableString, duration);
            } else {
                sToast = Toast.makeText(SimpleDynamicForm.getInstance(), text, duration);
            }
        }
